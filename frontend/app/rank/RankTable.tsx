...
export default function RankTable({ ymonth, searchType }: RankTableProps) {
        ...
        <Thead bg={headerBg}>
          <Tr>
            {visibleColumns.map((column) => (
              <Th key={column} isNumeric={column.includes('수')}>
                {column}
              </Th>
            ))}
          </Tr>
          ...
          {rankData.map((item, index) => (
            ...
            >
              {visibleColumns.includes('순위') && (
                <Td textAlign="center">
                  <Badge colorScheme={index < 3 ? 'blue' : 'gray'} borderRadius="full" px={2}>
                    {index + 1}
                  </Badge>
                </Td>
              )}
              {visibleColumns.includes('이름') && (
                <Td fontWeight={index < 5 ? 'bold' : 'normal'}>
                  <Text maxW={{ base: '150px', md: '100%' }}>{item?.company_nm || ''}</Text>
                </Td>
              )}
              ...
              {visibleColumns.includes('입사자 수') && (
                <Td isNumeric>
                  <HStack justifyContent="flex-end" spacing={1}>
                    <Text
                      color={searchType === 'new' ? 'green.500' : 'inherit'}
                      fontWeight={searchType === 'new' ? 'bold' : 'normal'}
                    >
                      {safeNumber(item?.subscriber_new)}
                    </Text>
                    {searchType === 'new' && index < 3 && (
              ...
              {visibleColumns.includes('퇴사자 수') && (
                <Td isNumeric>
                  <HStack justifyContent="flex-end" spacing={1}>
                    <Text
                      color={searchType === 'quit' ? 'red.500' : 'inherit'}
                      fontWeight={searchType === 'quit' ? 'bold' : 'normal'}
                    >
                      {safeNumber(item?.subscriber_quit)}
                    </Text>
                    {searchType === 'quit' && index < 3 && (
