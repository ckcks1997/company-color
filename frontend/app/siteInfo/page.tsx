...
function SiteInfo() {
  const infoText = `COMPANY COLOR 사이트는 신뢰할 수 있는 국민연금 데이터를 기반으로 기업 정보를 제공합니다.
...
`
  ...
  const siteFunction = `1. 기업명과 지역 기반 검색 기능
...
`
  ...
  const dataUpdate = `COMPANY COLOR의 데이터는 매월 25일 이후로 전월 데이터가 업데이트 됩니다.
    ...
    <Box p={8} maxWidth="1200px" margin="0 auto">
      <VStack spacing={8} align="stretch">
        ...
        >
          ...
          <Text style={{ whiteSpace: 'pre-wrap', lineHeight: '2' }}>{policyText}</Text>
        </Box>
      </VStack>
    </Box>
  )
