...
async def get_rank_info(ymonth: str, type: str, db: SessionDep):
    ...
    """
    if not ymonth:
        raise BusinessException(detail="조회 년월을 입력해주세요")

    if not type or type not in ['new', 'quit']:
        raise BusinessException(detail="유효한 순위 유형을 입력해주세요 (new 또는 quit)")
...
async def get_dart_info(name: str, db: SessionDep):
    ...
    """
    if not name:
        raise BusinessException(detail="회사명을 입력해주세요")

    # DB에서 회사 코드 조회 (정규화는 crud에서 처리)
